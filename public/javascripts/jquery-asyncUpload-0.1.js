...
(function($) {
    $.fn.makeAsyncUploader = function(options) {
        return this.each(function() {
            ...
            container.append($("<span id='" + id + "_uploading'>uploading <input class='upload_cancel_btn' type='submit' value='Cancel'/></span>"));
            container.append($("<div class='ProgressBar'> <div>&nbsp;</div> </div><div class='clear'></div>"));
            ...
            var defaults = {
                ...
                file_queued_handler: function(file) { start_upload();swfu.startUpload(); },

                // Called when a file doesn't even begin to upload, because of some error
                file_queue_error_handler: function(file, code, msg) {$("span[id$=_completedMessage]", container).html(msg); },

                // Called when an error occurs during upload
                upload_error_handler: function(file, code, msg) {
                    // $("span[id$=_completedMessage]", container).html("Sorry, upload failed. Please try again");
                    notice(msg)
                    $j('.swfupload').show();
                    swfu.setButtonDimensions(124, 18);
                    set_values();
                },
                ...
                upload_success_handler: function(file, response) {
                    if(response != "error"){
                      if($j('#uploaded_pic').length > 0){
                        ...
                        $j('#uploaded_pic').html(response);
                        $j('.swfupload').hide();
                      }
                      if($j('#user_avatar').length > 0){
                        $("span[id$=_completedMessage]", container).html("").hide();
                        $j('#user_avatar').html(response);
                      }
                      if($j('#company_avatar').length > 0){
                        javascript:location.reload(true)
                      }
                      swfu.setButtonDimensions(124, 18);
                      set_values();
                    }else{
                      notice("you have crossed the uploading limit ")
                      $("span[id$=_completedMessage]", container).html("You have crossed the uploading limit");
                    }
                    if($j('#user_avatar').length > 0){
                      $("span[id$=_completedMessage]", container).html("").hide();
                      $j('#user_avatar').html(response);
            ...
            $("span[id$=_uploading] input[type='submit']", container).click(function() {
