...
function O(k,o){
  ...
  if(F){
    ...
    k=6048E5/rb;
    q=2592E6/rb;
    var ma=31556952E3/rb,s=[["second",G,[1,2,5,10,15,30]],["minute",V,[1,2,5,10,15,30]],["hour",Ba,[1,2,3,4,6,8,12]],["day",U,[1,2]],["week",k,[1,2]],["month",q,[1,2,3,4,6]],["year",ma,null]],W=s[6],ba=W[1],ia=W[2];
    ...
    for(Ba=ia[sc]();G<fa&&r<Ca;){
      ...
      else if(!A&&(ba===U||ba===k))G=Dc(o,V,Ba+r*s*(ba===U?1:7));else G+=ba*s;
      r++
    }
    ta.push(G);
...
function Aa(){
  ...
  fb=ua;
  ua=oa/(fa-ca||1);
  if(!Ga)for(k in t)for(o in t[k])t[k][o].cum=t[k][o].total;if(!v.isDirty)v.isDirty=
    ca!==Gb||fa!==Ld
    }
    function Qa(k){
  k=(new x(k)).render();
...
y.prototype={
  addLabel:function(){
    ...
    if(A===Va)this.label=M(k)&&r&&o.enabled?ga.text(k,0,0).attr({
      align:o.align,
      rotation:o.rotation
      }).css(q).add(gc):null;else A&&A.attr({
      text:k
    }).css(q)
      },
...
var t=ga.g("tooltip").attr({
  ...
  "stroke-width":na
}).add(t).shadow(m.shadow),N=ga.text("",Qa+Ra,ja(Ma.fontSize)+Qa+Ra).attr({
  zIndex:1
}).css(Ma).add(t);
t.hide();
return{
...
gb=w.spacingBottom,wb=w.spacingLeft,Ob,Kb,wc,ha,Hb,sb,Y,Ub,mb,Vb,ya,Pd,Uc,xc,Ya,Ua,jd,Pc,Xc,ud,vd,Yc,p=this,ae=(R=w.events)&&!!R.click,wd,kc,uc,nd,bc,Sd,xd,xa,Ca,jc,Rc,Qc,od,Wb,Xb,tc,lc=w.showAxes,Sc=0,db=[],Mb,Ia=[],qa,ga,kd,Od,ld,pd,vc,sd,td,rd,md,Td,be=function(m){
  ...
  if(!Ja){
    ...
    L.legendItem=
    Ja=ga.text(O.labelFormatter.call(L),0,0).css(L.visible?Za:Ka).on("mouseover",function(){
      L.setState(Bb);
      Ja.css(Ga)
...
function na(){
  ...
  if(oa||ua){
    Wb+=2*z;
    Xb+=2*z;
    if($)Wb>0&&Xb>0&&$.animate($.crisp(null,null,null,Wb,Xb));else $=ga.rect(0,0,Wb,Xb,O.borderRadius,oa||0).attr({
...
pd=function(){
  ...
  if(m||i)if(Xc)Xc.animate(Xc.crisp(null,null,null,Ya-Q,Ua-Q));else Xc=ga.rect(Q/
    2,Q/2,Ya-Q,Ua-Q,w.borderRadius,m).attr({
    stroke:w.borderColor,
    "stroke-width":m,
    fill:i||jb
    }).add().shadow(w.shadow);
  if(y)if(ud)ud.animate(na);else ud=ga.rect(Y,ha,Ca,xa,0).attr({
    fill:y
  }).add().shadow(w.plotShadow);
  if(x)if(vd)vd.animate(na);else vd=ga.image(x,Y,ha,Ca,xa).add();
  if(w.plotBorderWidth)if(Yc)Yc.animate(Yc.crisp(null,Y,ha,Ca,xa));else Yc=ga.rect(Y,ha,Ca,xa,0,w.plotBorderWidth).attr({
...
if(R)for(wd in R)Sa(p,wd,R[wd]);p.options=a;
p.series=Ia;
...
p.get=function(m){
  var i,y,x;
  for(i=0;i<db.length;i++)if(db[i].options.id===m)return db[i];for(i=0;i<Ia.length;i++)if(Ia[i].options.id===m)return Ia[i];for(i=0;i<Ia.length;i++){
    x=Ia[i].data;
    for(y=0;y<
    ...
    attr:function(a,b){
      ...
      if(Pb(a)&&M(b)){
        ...
        if(l&&/^(x|y|r|start|end|width|height|innerR)/.test(c)){
          ...
          this[c]=
          d;
          if(f.tagName==="SPAN")this.updateTransform();else g[{
            x:"left",
            y:"top"
          }
          [c]]=d
...
getAttribs:function(){
  ...
  for(f=h.length;f--;){
    ...
    e=false;
    if(g.options)for(J in n)if(M(b[n[J]]))e=true;if(e){
      l=[];
      c=b.states||{};
...
drawTracker:function(){
  ...
  d,e,f=+new Date,g=a.options.cursor,h=g&&{
    cursor:g
  },j;
  u(a.data,function(l){
    e=l.tracker;
...
drawDataLabels:function(){
  ...
  l=this.radiusY,n=j>0,J=this.center[1],D=[[],[]],aa,E,da,w,R=2,B;
  if(d.enabled){
    ...
    for(a=D[0][0]&&D[0][0].dataLabel&&ja(D[0][0].dataLabel.styles.lineHeight);R--;){
      ...
      if(za>da){
        ...
        else if(da<za-B+Da&&K[B]!==null)Da=da-za+B;else for(;K[Da]===null;)Da++;
        S.push({
        i:Da,
        y:K[Da]
        });
