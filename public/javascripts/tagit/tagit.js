...
(function($) {
    $.widget("ui.tagit", {
        ...
        _addTag: function(value) {
            ...
            var tag = "";
            tag = '<li class="tagit-choice">' + value + '<a id=close-' + value + ' class="tagit-close">x</a></li>';
        ...
        _initialTags: function() {
            if (this.options.initialTags.length != 0) {
                var tagsAll = this.options.initialTags
                var tagsArray = tagsAll.split(",")
                var len = tagsArray.length;
                for(var i=0; i<len; i++) {
                    if (!this._exists(tagsArray[i]))
                        this._addTag(tagsArray[i]);
                }
