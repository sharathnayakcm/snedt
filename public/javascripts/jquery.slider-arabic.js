  ...
  jSlider.prototype = {
    ...
    generateScale: function(){
      if( this.settings.scale && this.settings.scale.length > 0 ){
        ...
        for( var i=0; i < s.length; i++ ){
          str += '<span style="right: ' + i*prc + '%">' + ( s[i] != '|' ? '<ins>' + s[i] + '</ins>' : '' ) + '</span>';
        };
        return str;
      } else return "";
    ...
    redrawLabels: function( pointer ){
      ...
      function setPosition( label, sizes, prc ){
        ...
        label.o.css({ right: prc + "%", marginLeft: sizes.margin, left: "auto" });
        if( sizes.left ) label.o.css({ right: "auto", left: 0 });
        return sizes;
      }
