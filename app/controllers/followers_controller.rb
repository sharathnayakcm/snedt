class FollowersController < ApplicationController
  ...
  def follow
    ...
    follower = User.find_by_id(params[:following])
    load_followers
    respond_to do |format|
      format.html {
        flash[:notice] = "You are now following #{follower.user_name}"
        redirect_to "/#{params[:next_url]}"
      }
      format.js {
        render :update do |page|
          page.replace_html "action_1_#{params[:following]}", link_to_remote('Unfollow',
            :url => unfollow_followers_path(:following => params[:following]),
            :html => {:class => 'button'})
          ...
          page << "$j('.follower_heading').html('You have #{followers_count - friends_count} Follower(s)!')"
          page << "setTimeout(\"$j('#follower_list_#{params[:following]}').hide()\", 3000);"
        end
      }
    end
  end
  ...
  def unfollow
    ...
    respond_to do |format|
      format.html {
        flash[:notice] = "You are no longer following #{follower.user_name}"
        redirect_to "/#{params[:next_url]}"
      }
      format.js {
        render :update do |page|
          page.replace_html "action_1_#{params[:following]}", link_to_remote('Follow',
            :url => follow_followers_path(:following => params[:following]),
            :html => {:class => 'button'})
          page << "$j('#followers_list_count').html(#{followers_count - friends_count})"
          page << "$j('#followings_list_count').html(#{followings_count - friends_count})"
          page << "$j('#friends_list_count').html(#{friends_count})"
          page << "$j('.no_users').html('You have #{friends_count} followings')"
          page << "$j('.follower_heading').html('You have #{followers_count - friends_count} Follower(s)!')"
          page << "$j('#follower_list_#{params[:following]}').show();"
        end
      }
    end
  end
  ...
  def streams
    @streams = current_user.followers_streams
    @no_post_panel = true
  ...
  def followers_filter_custom_streams
    @custom_tab = current_user.custom_tabs.find_by_id(params[:id])
    @streams = current_user.followers_streams
    @filter = true
    @by = params[:by]
    ...
    if params[:by]
      if params[:by] == "all"
        ...
        edintity = current_user.followers_edintity_streams
