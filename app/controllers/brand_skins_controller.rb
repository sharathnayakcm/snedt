class BrandSkinsController < ApplicationController
  ...
  def create
    @brand = Brand.find(params[:brand_id])
    skins = @brand.skins.find_all_by_name(params[:name].strip)
    if params[:name].strip.blank?
      ...
      @brand.create_skin(params[:name].strip)
  ...
  def delete_skin
    @brand = Brand.find(params[:brand_id])
    skin = @brand.skins.find_by_id(params[:skin_id])
  ...
  def upload_image
    ...
    responds_to_parent do
      render :update do |page|
        ...
        page << "$j('#noImage').show();"
        page.insert_html :bottom,  "background_image_#{skin_style.panel_type.id}", :partial => "skins/brand/background_images", :locals => {:skin_style => skin_style, :url => base_url, :image => image}
      end
    end
  end
